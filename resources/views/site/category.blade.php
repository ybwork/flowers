						...
						<p>
							@if(array_key_exists($product->id, $cart))
								<form id="ajax-form" action="{{ route('product_delete_from_cart') }}" method="POST">
									...
									<button id="ajax-button" class="form-group btn-danger" type="submit">Delete from cart</button>
								</form>
							@else
								<form id="ajax-form" action="{{ route('product_add_to_cart') }}" method="POST">
									...
									<input type="hidden" name="_token" value="{{ csrf_token() }}">
									<input type="hidden" name="product_id" value="{{ $product->id }}">
									<button id="ajax-button" class="form-group btn-success" type="submit">Add to cart</button>
								</form>
							@endif
						</p>
			        </div>
